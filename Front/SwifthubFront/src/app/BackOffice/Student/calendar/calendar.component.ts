...
export class CalendarComponentStudent implements OnInit {
  ...
  loadAppointments() {
    this.appointmentService.getEmployeeList().subscribe({
      next: (appointments: any[]) => {
        const userAppointments = appointments.filter(appointment => appointment.studentId === this.user.id && appointment.status ==="CONFIRMED_BY_TEACHER");
